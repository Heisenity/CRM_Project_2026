/**
 * Deployment Admin Seeder (TypeScript)
...
function generateAdminId(): string {
  ...
  return `ADMIN_${timestamp}`;
}

// Save credentials to file for deployment reference
function saveCredentialsToFile(credentials: AdminCredentials): string {
...
function displayCredentials(credentials: AdminCredentials): void {
  console.log('\n' + '='.repeat(65));
...
export async function seedDeploymentAdmin(): Promise<AdminCredentials | null> {
  try {
    ...
    const email = process.env.INITIAL_ADMIN_EMAIL || `admin@${process.env.COMPANY_DOMAIN || 'company.com'}`;
    const password = generateSecurePassword(16); // 16 character secure password
